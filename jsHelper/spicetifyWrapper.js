...
(function hotloadWebpackModules() {
    ...
    Object.assign(Spicetify, {
        ...
        GraphQL: {
            ...
            get ResponseDefinitions() {
                return Object.fromEntries(
                    Object.entries(Spicetify.GraphQL.Definitions).filter(([, value]) =>
                        value.definitions.every((def) => def.kind !== "OperationDefinition")
                    )
                );
            },
        ...
        ReactComponent: {
            ...
            Slider: wrapProvider(
                functionModules.find((m) => m.toString().includes("onStepBackward") && !m.toString().includes("volume"))
            ),
            ...
            PanelHeader: functionModules.find(
            ...
            Chip: modules.find(
                (m) =>
                    m?.render?.toString().includes("invertedDark") && m?.render?.toString().includes("isUsingKeyboard")
            ),
            ...
            Cards: {
                ...
                CardImage: functionModules.find(
                    (m) => m.toString().includes("isHero") && m.toString().includes("withWaves")
                ),
    ...
    if (localeModule) {
        const createUrlLocale = functionModules.find(
            (m) =>
                m.toString().includes("has") && m.toString().includes("baseName") && m.toString().includes("language")
        );
        Spicetify.Locale = {
    ...
    if (dropdownChunk) {
        Spicetify.ReactComponent.Dropdown = Object.values(require(dropdownChunk[0])).find(
            (m) => typeof m === "function"
        );
    }
    ...
    const infiniteQueryChunk = chunks.find(
        ([_, value]) =>
            value.toString().includes("fetchPreviousPage") && value.toString().includes("getOptimisticResult")
    );
    ...
    (async function bindColorExtractor() {
        ...
        if (!imageAnalysis) {
            let chunk = chunks.find(
                ([, value]) =>
                    (value.toString().match(/[\w$]+\.isFallback/g) || value.toString().includes("colorRaw:")) &&
            ...
            if (!chunk) {
                ...
                chunk = chunks.find(
                    ([, value]) =>
                        value.toString().match(/[\w$]+\.isFallback/g) && value.toString().match(/.extractColor/g)
                );
            }
...
Spicetify.getAudioData = async (uri) => {
    ...
    return await Spicetify.CosmosAsync.get(
        `wg://audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`
    );
};
...
Spicetify._getStyledClassName = (args, component) => {
    ...
    const customEntries = Object.entries(element).filter(
        ([key, value]) =>
            customKeys.some((k) => key.toLowerCase().includes(k)) && typeof value === "string" && value.length
    );
                ...
                React.cloneElement(
                    ...
                    React.cloneElement(
                        link,
                        {
                            ...
                            className: conditionalAppend(
                                "link-subtle main-yourLibraryX-navLink",
            ...
            obj = React.cloneElement(
                ...
                React.cloneElement(
                    link,
                    {
                        ...
                        isActive: (e, { pathname: t }) => t.startsWith(appLink),
                        className: conditionalAppend(
                            "link-subtle main-navBar-navBarLink",
...
(function waitForPanelAPI() {
    ...
    class ErrorBoundary extends Spicetify.React.Component {
        ...
        componentDidCatch(error, info) {
            ...
            Spicetify.showNotification(
                `Something went wrong in panel ID "${this.props.id}" ${
    ...
    Spicetify.Panel = {
        ...
        render: () => {
            ...
            return (
                (!Spicetify.Panel.reservedPanelIds[currentPanel] && contentMap.get(Spicetify.Panel.currentPanel)) ||
                null
            );
        },
        get currentPanel() {
    ...
    try {
        ...
        if (semver !== version) {
            ...
            content.innerHTML = `
				...
				<p class="spicetify-update-space">새로운 기능과 버그 수정을 받으려면 Spicetify를 업데이트하세요.</p>
				<p> 현재 버전: ${version} </p>
				<p> 최신 버전:
            ...
            (function waitForTippy() {
                ...
                for (const instance of tippy) {
                    instance.reference.addEventListener("click", () => {
                        ...
                        instance.setContent("복사완료!");
                        setTimeout(() => instance.setContent("클릭하여 복사하기"), 1000);
                    });
                }
            })();

            const updateModal = {
                title: "Spicetify 업데이트",
                content,
                isLarge: true,
            };
