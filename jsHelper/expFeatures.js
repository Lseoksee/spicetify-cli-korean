...
.spicetify-exp-features input.search {
    ...
    (function waitForRemoteConfigResolver() {
        ...
        let remoteConfiguration =
            Spicetify.RemoteConfigResolver?.value.remoteConfiguration || Spicetify.Platform?.RemoteConfiguration;
        let setOverrides = () => {};
...
<input type="text" class="search" placeholder="검색하기">
        ...
        search.oninput = () => {
            ...
            for (const row of rows) {
                ...
                row.style.display =
                    row.textContent.trim().toLowerCase().includes(query) || row.id.toLowerCase().includes(query)
                        ? "flex"
                        : "none";
            }
        };
