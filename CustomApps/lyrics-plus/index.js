...
const CONFIG = {
	...
	providers: {
		...
		musixmatch: {
			...
			desc: "Spotify와 완벽하게 호환이 됩니다. 가사를 원활하게 불러오지 못할 경우 <code>토큰 초기화</code> 버튼을 눌러주세요. 경우에 따라 자체 CORS 프록시로 변경해야 할 수도 있습니다.",
...
class LyricsContainer extends react.Component {
	...
	async translateLyrics(language, lyrics, targetConvert) {
		...
		Spicetify.showNotification("변환중...", false, 1000);
		if (!this.translator) {
		...
		try {
			if (language === "ja") {
				...
				if (targetConvert === "cn") {
					Spicetify.showNotification("변환이 필요 없습니다", false, 1000);
					return lyrics;
				}
			...
			Spicetify.showNotification("변환중...", false, 0);
			return res;
		} catch (error) {
			Spicetify.showNotification("변환 오류!", true);
			console.error(error);
		}
	}
	...
	processLyricsFromFile(event) {
		...
		if (file[0].size > 1024 * 1024) {
			Spicetify.showNotification("파일이 너무 큽니다.", true);
			return;
		}
	...
	componentDidMount() {
		...
		this.configButton = new Spicetify.Menu.Item("가사 설정", false, openConfig, "lyrics");
						...
						"button",
						{
							...
							onClick: () => {
								...
								if (this.state.isCached) {
									...
									Spicetify.showNotification("저장된 가사를 삭제했습니다");
								} else {
